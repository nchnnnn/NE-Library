...
async function getAttendanceLogs(user_type, status, start_date, end_date, limit = 10, offset = 0, search = '', college_id = '') {
    let baseQuery = `
    ...
    if (user_type === 'student') conditions.push("u.id IS NOT NULL AND su.id IS NULL");
    if (user_type === 'staff') conditions.push("su.id IS NOT NULL AND u.id IS NULL");
    ...
    if (search) {
        conditions.push("(u.first_name LIKE ? OR u.last_name LIKE ? OR u.student_number LIKE ? OR su.first_name LIKE ? OR su.last_name LIKE ? OR su.employee_id LIKE ?)");
...
async function getAttendanceStats(start_date, end_date, college_id, program_id) {
    const conditions = [];
    const params = [];
    ...
    // Today's entries
    const todayConditions = ["DATE(al.time_in) = CURDATE()", ...conditions.filter(c => !c.includes("time_in"))];
    ...
    // This week's entries
    const weekConditions = ["YEARWEEK(al.time_in) = YEARWEEK(NOW())", ...conditions.filter(c => !c.includes("time_in"))];
    ...
    const [staffCount] = await db.query(
        ...
        params.slice(0, conditions.filter(c => !c.includes("sec") && !c.includes("p.")).length)
    );
